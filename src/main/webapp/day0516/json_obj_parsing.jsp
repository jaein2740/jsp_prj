		...
		$.ajax({
			...
			success:function(jsonObj){
				...
				if(!jsonObj.resultFlag){
					$("#deptTab tbody").append("<tr><td colspan='3'>부서정보가 존재하지 않습니다.</td></tr>");
...
function searchEmp(deptno){
	...
	$.ajax({
		...
		success: function( jsonObj ){
			...
			if(!jsonObj.resultFlag){
				$("#empTab tbody").append("<tr><td colspan='6'>사원이 존재 하지 않습니다</td></tr>");
			}//end if
			
			var createTr;
			$.each(jsonObj.data,function(i,jsonTemp){
				createTr="<tr><td>"+(i+1)+"</td><td>"+jsonTemp.empno+"</td><td>"+jsonTemp.ename+"</td><td>"
