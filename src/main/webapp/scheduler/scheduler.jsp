...
<style type="text/css">
 ...
 .calTab > tbody > tr > td{width: 120px; height: 80px; 
 		border: 1px solid #CECECE; vertical-align: top; padding-left: 5px; font-weight: bold;}
...
<% // 1일의 요일 출력 -1일을 출력하기전 공백 설정
	...
	int blackTdCnt=cal.get(Calendar.DAY_OF_WEEK);
	String dayColorCss="";//요일별 색 설정
	String todayCss="";//오늘의 바닥색 설정
	
	//log("------blackTdCnt------"+blackTdCnt);
	for(int blackTd=1; blackTd<blackTdCnt; blackTd++){
	%>
	...
	for(int tempDay =1; ; tempDay++){
		...
		if(cal.get(Calendar.DAY_OF_MONTH) != tempDay){
			//마지막 날이면 빈 td를 만들고, 반복문 빠져 나간다.
			for(int blackTd=cal.get(Calendar.DAY_OF_WEEK); blackTd<Calendar.SATURDAY+1;blackTd++){
				%>
				<td class ="blackTd"></td>
				<%
			}//end for
