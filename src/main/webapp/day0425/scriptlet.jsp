...
%>
<div>
1~100까지 합은 <span class="span"><%=sum %></span>입니다
</div>
...
for(int i =1; i<10;i++){%>
	<tr>
	<%for(int j=2; j<10;j++){%>
	<td class="rnrneks" onclick="rnrneks(<%=j%>,<%=i%>)"><%=j%> X <%=i%></td>
	<%}//end for%>
	</tr>
<%}//end for%>
...
list.add(new DataDTO("img_3.jpg","이건못참지!!!","오전에 수고하셨고 점심","https://www.yogiyo.co.kr/mobile/#/"));
%>
<div>
<%for(DataDTO dto : list){ %>
<div class="card" style="width: 18rem; float:left;margin-right: 10px ">
  <img style="width:286px; height: 160px" src="http://192.168.10.72/jsp_prj/common/images/<%=dto.getImg()%>" class="card-img-top" alt="...">
  <div class="card-body">
...
<%
	...
	int lastDayOfMonth=ld.withDayOfMonth(ld.lengthOfMonth()).getDayOfMonth();
...
<option value="<%=nowYear+tempYear %>"<%=selected%>><%=nowYear+tempYear %></option>
<%}%>
...
<% for (int month = 1; month <= 12; month++) {
     String selectedMonth = (month == nowMonth) ? " selected='selected'" : "";
%>
...
for(int day =1; day<nowDate+1; day++){ 
	String selectedDay = (day == nowDate) ? " selected='selected'" : "";
%>
